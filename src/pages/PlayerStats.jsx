...
export default function PlayerStats() {
  ...
  const totalPlants = stats.reduce((a,b)=>a+b.plants,0);
  const totalDefuses = stats.reduce((a,b)=>a+b.defuses,0);
  const firstBloods = stats.reduce((a,b)=>a+b.first_blood,0);
  ...
  const kpis = [
    ...
    { label: 'Defuses', value: totalDefuses },
    { label: 'First Bloods', value: firstBloods },
  ];
  ...
  const radarData = data.players.map(p => {
    ...
    const acs = ps.length ? (ps.reduce((a,b)=>a+b.acs,0)/ps.length).toFixed(1) : 0;
    return { player: p.player_name, kills, deaths, assists, kd, acs };
  });

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Player Stats Dashboard</h1>
      ...
      <h2 className="mt-6 font-semibold mb-4">Radar Stats par joueur</h2>
      <ResponsiveContainer width="100%" height={400}>
        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
          ...
          <Radar name="ACS" dataKey="acs" stroke="#a4de6c" fill="#a4de6c" fillOpacity={0.3} />
        </RadarChart>
      </ResponsiveContainer>
    </div>
  );
